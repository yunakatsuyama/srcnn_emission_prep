...
train_cams_data = np.load("data/cams_conserv_xe_aug_train.npy")
train_eagrid_data = np.load("data/eagrid_dataset_aug_train.npy")
train_bilinear_data = np.load("data/cams_interp_xe_aug_train.npy") # omly for comparizon 

test_cams_data = np.load("data/cams_conserv_xe_aug_test.npy")
test_eagrid_data = np.load("data/eagrid_dataset_aug_test.npy")
test_bilinear_data = np.load("data/cams_interp_xe_aug_test.npy")
...
for i in range(50):
    class SRDataset(Dataset):
        def __init__(self, cams, eagrid, bilinear, indices=None):
            if not (len(cams) == len(eagrid) == len(bilinear)):
    ...
    def split_test_into_val_test(n_samples, val_ratio=0.5, seed=42):
        np.random.seed(seed)
    ...
    val_idx, test_idx = split_test_into_val_test(len(test_cams_data))
    print(f'train, vali, test {len(train_cams_data), len(val_idx), len(test_idx)}')
    ...
    epoch_loss_val = 0.0
    epoch_loss_test = 0.0
    epoch_loss_test_bili = 0.0
    for epoch in range(epoch):

    # == Train ======================= 
        net.train()
        for batch_low, batch_high ,batch_bili, idx_batch in train_loader:   # train_high is a dataloader
    ...
    # print(L.item())
        ...
        # == Validation ===================
        net.eval()
        for val_batch_low, val_batch_high ,batch_bili, idx_batch in val_loader:
    ...
    for test_batch_low, test_batch_high, batch_bili, idx_batch in test_loader:
        test_batch_low = test_batch_low.unsqueeze(1).float().to(device)
        test_batch_high = test_batch_high.unsqueeze(1).float().to(device)
        bilinear_batch = batch_bili.unsqueeze(1).float().to(device)
        
        Z_test = net(test_batch_low)
        L_test = criterion(Z_test, test_batch_high)
        L_bili = criterion(bilinear_batch, test_batch_high)


        all_Z_test.append(Z_test.detach().cpu())
        all_bilinear.append(bilinear_batch.detach().cpu())

        epoch_loss_test += L_test.item() * test_batch_low.size(0)
        epoch_loss_test_bili += L_bili.item() * test_batch_low.size(0)

    L_test_last = epoch_loss_test / len(test_dataset)
    L_test_last_bili = epoch_loss_test_bili / len(test_dataset)
    ...
    all_Z_test = torch.cat(all_Z_test, dim=0).numpy()
    all_bilinear = torch.cat(all_bilinear, dim=0).numpy()


    #mse_model = np.mean((all_Z_test - test_eagrid_data[test_idx])**2)
    #mse_bilinear = np.mean((all_bilinear - test_eagrid_data[test_idx])**2)
    #print("MSE model:", mse_model)
    #print("MSE bilinear:", mse_bilinear)
    print(f'L_test {L_test_last}')    
    print(f'L test bili {L_test_last_bili}')

    test.append(L_test_last)
    bili.append(L_test_last_bili)


data = np.column_stack((test, bili))
np.savetxt(folder + 'output.csv', data, delimiter=',')


#L_filename = folder + 'srcnn.csv'
...
#eagrid_sorted = test_eagrid_data[test_idx][sorted_idx]
#cams_sorted = test_cams_data[test_idx][sorted_idx]


#np.save(folder + "SRCNN_output_test.npy", all_Z_test_sorted)
#np.save(folder + "bilinear_test.npy", all_bilinear_sorted)
#np.save(folder + "ground_truth_test.npy", eagrid_sorted)
#np.save(folder + "input_cams_test.npy", cams_sorted)
